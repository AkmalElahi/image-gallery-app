...
import { selectwallpaperRandom, selectwallpaperLoading } from '../../redux/wallpapers/wallpaper.selector'
...
import { selectAlbumRandom, selectalbumLoading } from '../../redux/albums/albums.selector';
...
const RandomTab = ({ navigation, onScroll, getAlbums, getWallpapers, currentTab, wallpapers, albums, albumsLoading, wallpapersLoading }) => {
    ...
    const getData = () => {
        if (active === 'wallpaper') {
            getWallpapers({ type: 'random', page: wapllpaperPage })
            return
        }
        if (active === 'album') {
            getAlbums({ type: 'random', page: albumPage })
            return
        }
    ...
    return (
        ...
        <View style={{ flex: 1, backgroundColor: colors.background }}>
            ...
            {/* {console.log("ALBUM PAGE", albumPage, "WALLPAPER PAGE", wapllpaperPage, 'random')} */}
...
const mapStateToProps = createStructuredSelector({
    wallpapers: selectwallpaperRandom,
    albums: selectAlbumRandom,
...
export default connect(mapStateToProps, mapDispatchToProps)(RandomTab)